...
- `d`: this means that the function is returning $f(x)$, i.e., the probability density function value given the coordinate $x$; and
...
| Prefix | What it Does |
| ------ | ------------ | 
| `d` | returns the pdf $f(x)$ or pmf $p(x)$ |
| `p` | returns the cdf $F(x)$ |
| `q` | returns the inverse cdf $F^{-1}(x)$ |
...
Assuming a one-to-one relationship between $x$ and $q = F(x)$, it is the case that $x = F^{-1}(F(x))$. So, for instance,
```
...
Plot the probability mass function $p(x)$ for a binomial distribution with $n = 20$ and $p = 0.5$. Does it look like a normal distribution, at least in general shape? Next, make two more pmf plots, one with $p = 0.1$ and one with $p = 0.9$. Do these pmfs have a similar shape to a normal distribution? (Note that what you see here is related to whether or not you can perform a population proportion hypothesis test, which requires that the binomial pmf has, approximately, the shape of a normal distribution.
```{r}
# FILL ME IN
```
...
Plot the probability mass function $p(x)$ for a Poisson distribution with $\lambda = 2$ and $\lambda = 10$. You should see "issues" similar to what you observed with you completed Question 8. (To give away a result: as $\lambda$ gets larger, the Poisson pmf adopts a shape more and more like that of a normal distribution.)
```{r}
# FILL ME IN
```
...
Try this yourself with the simplest of all continuous distributions, the uniform. The uniform has two parameters (`a`, the minimum value, and `b`, the maximum value), and the pdf $f(x)$ is
$$f(x) = \frac{1}{b-a} \,.$$
